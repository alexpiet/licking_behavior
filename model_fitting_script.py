...
if __name__ == '__main__': # Don't understand why you need this part
    ...
    include_reward=True, num_reward_params=10*duration,reward_duration =duration, reward_sigma = 0.1 ,rewardsdt=rewardsdt,
    include_flashes=False, num_flash_params=10,flash_duration=0.750, flash_sigma = 0.025, flashesdt=flashesdt,
    ...
    # Do Optimization
    inital_param = np.concatenate(([-.5],np.ones((duration*10,))))
    ...
    # Save res
    filepath = "reward_glm__"+str(experiment_id) + "_" +str(duration) 
    file_temp = open(filepath,'wb')
    pickle.dump([res,duration], file_temp)
    file_temp.close()
