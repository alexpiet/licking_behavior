...
def running_wrapper_func(params):
    return fit_tools.licking_model(np.concatenate(([-.5],np.zeros((10,)),np.zeros((5,)))), licksdt, stop_time, post_lick=True,include_running_speed=True, num_running_speed_params=5,running_speed=running_speed)[0]
...
def wrapper(params):
    return fit_tools.licking_model(np.concatenate(([-.5],np.zeros((10,)),np.zeros((5,)))), licksdt, stop_time, post_lick=True,include_running_speed=True, num_running_speed_params=5,running_speed=running_speed)
