...
nll, latent = fit_tools.licking_model(np.concatenate(([-.5],np.zeros((10,)))), licksdt, stop_time, post_lick=True,running_speed=True)

# Wrapper function for optimization that only takes one input
def running_wrapper_func(params):
    return fit_tools.licking_model(params, licksdt, stop_time, post_lick=True,running_speed=True)[0]
...
def wrapper(params):
    return fit_tools.licking_model(params, licksdt, stop_time, post_lick=True,running_speed=True)
